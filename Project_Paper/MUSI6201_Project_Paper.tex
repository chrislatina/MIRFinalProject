...
The dataset used for training and testing the machine learning model plays a significant role in its results. Many datasets that exist for genre classification only contain the minimum amount of information, namely the audio snippet and a label. Recently, researchers are taking effort to compile more complete and accurate datasets in different ways. One approach is to supplement the existing data by providing music-related metadata information such as the song title, album name, year and even price of the release.
