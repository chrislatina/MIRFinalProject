...
years = yearsLabel(I(:,1:K));
k_dist = B(:,1:K);
f_dist = (1- (k_dist ./ repmat(sum(k_dist,2),1,K))) ./ (K-1);
estimated_years = mean(f_dist .* years,2) * K;
...
for j = 1: size(I,1)
    ...
    estimated_genres(j,1) = all_labels(min_dist_inds(min_dist_ind));
end
